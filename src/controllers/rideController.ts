...
export async function aiSearchRides(req: AuthedRequest, res: Response) {
  const {
    source_lat, source_lng, dest_lat, dest_lng,
    ride_date, seats_needed,
    source_city, dest_city
  ...
  try {
    ...
    const directResult = await pool.query(
      ...
      WHERE (
        (source_distance < 15 OR ($7::text IS NOT NULL AND LOWER(source) LIKE '%' || $7 || '%') OR EXISTS (SELECT 1 FROM stops s WHERE s.parent_id = sub.ride_id AND s.parent_type = 'RIDE' AND ((6371 * acos(LEAST(GREATEST(cos(radians($1)) * cos(radians(s.latitude)) * cos(radians(s.longitude) - radians($2)) + sin(radians($1)) * sin(radians(s.latitude)), -1), 1))) < 15 OR ($7::text IS NOT NULL AND LOWER(s.city_name) LIKE '%' || $7 || '%'))))
        AND 
        (dest_distance < 15 OR ($8::text IS NOT NULL AND LOWER(destination) LIKE '%' || $8 || '%') OR EXISTS (SELECT 1 FROM stops s WHERE s.parent_id = sub.ride_id AND s.parent_type = 'RIDE' AND ((6371 * acos(LEAST(GREATEST(cos(radians($3)) * cos(radians(s.latitude)) * cos(radians(s.longitude) - radians($4)) + sin(radians($3)) * sin(radians(s.latitude)), -1), 1))) < 15 OR ($8::text IS NOT NULL AND LOWER(s.city_name) LIKE '%' || $8 || '%'))))
      )
      ORDER BY (source_distance + dest_distance) ASC`,
      [s_lat, s_lng, d_lat, d_lng, needed, ride_date || null, s_city, d_city]
    );

    const enrichStops = async (rides: any[]) => {
      return Promise.all(rides.map(async (r) => {
        const s = await pool.query('SELECT * FROM stops WHERE parent_id=$1 AND parent_type=\'RIDE\' ORDER BY stop_order ASC', [r.ride_id]);
        return { ...r, stops: s.rows };
      }));
    };

    if (directResult.rows.length > 0) {
      const ridesWithStops = await enrichStops(directResult.rows);
      const output = ridesWithStops.map(r => ({ match_type: 'DIRECT', ride: r }));
      return res.json(output);
    }

    // PHASE 2: CONNECTED / MULTI-HOP MATCH
    
    const ridesFromA = await pool.query(
      `SELECT * FROM (
        SELECT r.*, 
        u.full_name as driver_name, u.profile_photo as profile_image, u.phone as driver_phone,
        (SELECT COALESCE(AVG(rating), 0) FROM ratings WHERE rated_user = u.user_id) as driver_rating,
        v.model as vehicle_model,
        (6371 * acos(LEAST(GREATEST(cos(radians($1)) * cos(radians(r.source_lat)) * cos(radians(r.source_lng) - radians($2)) + sin(radians($1)) * sin(radians(r.source_lat)), -1), 1))) AS source_distance
        FROM rides r
        JOIN users u ON r.driver_id = u.user_id
        JOIN vehicles v ON r.vehicle_id = v.vehicle_id
        WHERE r.status = 'ACTIVE' AND r.available_seats >= $3 AND ($4::date IS NULL OR r.ride_date = $4)
      ) sub
      WHERE (source_distance < 30 OR ($5::text IS NOT NULL AND LOWER(source) LIKE '%' || $5 || '%') OR EXISTS (SELECT 1 FROM stops s WHERE s.parent_id = sub.ride_id AND s.parent_type = 'RIDE' AND ((6371 * acos(LEAST(GREATEST(cos(radians($1)) * cos(radians(s.latitude)) * cos(radians(s.longitude) - radians($2)) + sin(radians($1)) * sin(radians(s.latitude)), -1), 1))) < 30 OR ($5::text IS NOT NULL AND LOWER(s.city_name) LIKE '%' || $5 || '%'))))`,
      [s_lat, s_lng, needed, ride_date || null, s_city]
    );

    const ridesToB = await pool.query(
      `SELECT * FROM (
        SELECT r.*, 
        u.full_name as driver_name, u.profile_photo as profile_image, u.phone as driver_phone,
        (SELECT COALESCE(AVG(rating), 0) FROM ratings WHERE rated_user = u.user_id) as driver_rating,
        v.model as vehicle_model,
        (6371 * acos(LEAST(GREATEST(cos(radians($1)) * cos(radians(r.dest_lat)) * cos(radians(r.dest_lng) - radians($2)) + sin(radians($1)) * sin(radians(r.dest_lat)), -1), 1))) AS dest_distance
        FROM rides r
        JOIN users u ON r.driver_id = u.user_id
        JOIN vehicles v ON r.vehicle_id = v.vehicle_id
        WHERE r.status = 'ACTIVE' AND r.available_seats >= $3 AND ($4::date IS NULL OR r.ride_date = $4)
      ) sub
      WHERE (dest_distance < 30 OR ($5::text IS NOT NULL AND LOWER(destination) LIKE '%' || $5 || '%') OR EXISTS (SELECT 1 FROM stops s WHERE s.parent_id = sub.ride_id AND s.parent_type = 'RIDE' AND ((6371 * acos(LEAST(GREATEST(cos(radians($1)) * cos(radians(s.latitude)) * cos(radians(s.longitude) - radians($2)) + sin(radians($1)) * sin(radians(s.latitude)), -1), 1))) < 30 OR ($5::text IS NOT NULL AND LOWER(s.city_name) LIKE '%' || $5 || '%'))))`,
      [d_lat, d_lng, needed, ride_date || null, d_city]
    );

    const expandedRidesFromA = await enrichStops(ridesFromA.rows);
    const expandedRidesToB = await enrichStops(ridesToB.rows);

    let multiHopMatches = [];
    const distHelper = (lat1: number, lon1: number, lat2: number, lon2: number) => {
      const R = 6371;
    ...
    for (const r1 of expandedRidesFromA) {
      for (const r2 of expandedRidesToB) {
        if (r1.ride_id === r2.ride_id) continue;
        
        // Correct date comparison (Postgres returns JS Date objects for dates sometimes)
        const date1 = new Date(r1.ride_date).toISOString().split('T')[0];
        const date2 = new Date(r2.ride_date).toISOString().split('T')[0];
        
        if (date1 !== date2) continue; // For multihop MVP, assume same date
        
        const r1Dropoffs = [{lat: r1.dest_lat, lng: r1.dest_lng, name: r1.destination}, ...r1.stops.map((s:any) => ({lat: s.latitude, lng: s.longitude, name: s.city_name}))];
        const r2Pickups = [{lat: r2.source_lat, lng: r2.source_lng, name: r2.source}, ...r2.stops.map((s:any) => ({lat: s.latitude, lng: s.longitude, name: s.city_name}))];

        let connectionPoint = null;
        for (const drop of r1Dropoffs) {
          for (const pick of r2Pickups) {
            const d = distHelper(drop.lat, drop.lng, pick.lat, pick.lng);
            if (d < 30) { // 30km threshold for connection (increased to cover city boundaries safely)
              connectionPoint = { dropoff: drop.name, pickup: pick.name, distance: d };
              break;
            }
          }
          if (connectionPoint) break;
        }

        if (connectionPoint) {
            // Manual Stopover Pricing Logic:
            // 1. Clip r1 from user's pickup point to connection dropoff
            const r1Points = [{lat: r1.source_lat, lng: r1.source_lng, name: r1.source, stop_price: 0}, ...r1.stops.map((s:any) => ({lat: s.latitude, lng: s.longitude, name: s.city_name, stop_price: Number(s.stop_price)}))];
            const r1DestExtended = [...r1Points, {lat: r1.dest_lat, lng: r1.dest_lng, name: r1.destination, stop_price: Number(r1.base_price)}];
            
            // Find best pickup point for r1
            let r1MinDistToPickup = Infinity;
            let r1PickupCoord = r1Points[0];
            for (const pt of r1Points) {
                const dist = distHelper(pt.lat, pt.lng, s_lat, s_lng);
                if (dist < r1MinDistToPickup) {
                    r1MinDistToPickup = dist;
                    r1PickupCoord = pt;
                }
            }
            
            // Find connection dropoff point in r1
            let r1DropoffCoord = r1DestExtended[r1DestExtended.length - 1];
            for (const pt of r1DestExtended) {
                if (pt.name === connectionPoint.dropoff) {
                    r1DropoffCoord = pt;
                    break;
                }
            }
            
            const r1SegmentPrice = Math.max(0, r1DropoffCoord.stop_price - r1PickupCoord.stop_price);

            const r1Clipped = {
                ...r1,
                source: r1PickupCoord.name,
                destination: connectionPoint.dropoff,
                base_price: r1SegmentPrice
            };

            // 2. Clip r2 from connection pickup to user's destination
            const r2Points = [{lat: r2.source_lat, lng: r2.source_lng, name: r2.source, stop_price: 0}, ...r2.stops.map((s:any) => ({lat: s.latitude, lng: s.longitude, name: s.city_name, stop_price: Number(s.stop_price)}))];
            const r2DestExtended = [...r2Points, {lat: r2.dest_lat, lng: r2.dest_lng, name: r2.destination, stop_price: Number(r2.base_price)}];
            
            // Find connection pickup point in r2
            let r2PickupCoord = r2Points[0];
            for (const pt of r2Points) {
                if (pt.name === connectionPoint.pickup) {
                    r2PickupCoord = pt;
                    break;
                }
            }
            
            // Find user's destination point in r2
            let r2MinDistToDest = Infinity;
            let r2DestCoord = r2DestExtended[r2DestExtended.length - 1];
            for (const pt of r2DestExtended) {
                const dist = distHelper(pt.lat, pt.lng, d_lat, d_lng);
                if (dist < r2MinDistToDest) {
                    r2MinDistToDest = dist;
                    r2DestCoord = pt;
                }
            }

            const r2SegmentPrice = Math.max(0, r2DestCoord.stop_price - r2PickupCoord.stop_price);

            const r2Clipped = {
                ...r2,
                source: connectionPoint.pickup,
                destination: r2DestCoord.name,
                base_price: r2SegmentPrice
            };

           multiHopMatches.push({
             match_type: 'MULTI_HOP',
             connection_point: connectionPoint,
             ride1: r1Clipped,
             ride2: r2Clipped,
             total_price: r1SegmentPrice + r2SegmentPrice
           });
        }
      }
    }
    ...
    if (multiHopMatches.length > 0) {
      return res.json(multiHopMatches);
    }

    // PHASE 3: NEAREST DROP-OFF (Fallback)
    let nearestMatches = [];
    for (const r1 of expandedRidesFromA) {
       const r1Dropoffs = [{lat: r1.dest_lat, lng: r1.dest_lng, name: r1.destination}, ...r1.stops.map((s:any) => ({lat: s.latitude, lng: s.longitude, name: s.city_name}))];
       let minD = Infinity;
       ...
       for (const drop of r1Dropoffs) {
         const d = distHelper(drop.lat, drop.lng, d_lat, d_lng);
         if (d < minD) {
           minD = d;
           bestDrop = drop;
         }
       }
       if (minD < 100) { // Limit to 100km max distance
         nearestMatches.push({
           match_type: 'NEAREST_DROPOFF',
           ride: r1,
           dropoff_point: bestDrop?.name,
           distance_to_destination: Math.round(minD)
         });
       }
    }

    if (nearestMatches.length > 0) {
      nearestMatches.sort((a, b) => a.distance_to_destination - b.distance_to_destination);
      return res.json(nearestMatches.slice(0, 5)); // Return top 5
    }

    return res.json([]); 
  } catch (error: any) {
    console.error('Error in aiSearchRides:', error);
    res.status(500).json({ message: 'Internal server error' });
  }
}
